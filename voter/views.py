    ...
    if (request.method == 'POST'):
        ...
        if (User.objects.filter(username=voterid)).exists():
            ...
            if Voters.objects.filter(voterid_no=voterid):
                ...
                url = "http://2factor.in/API/V1/dc4e0f2a-2737-11ee-addf-0200cd936042/SMS/" + user_phone + "/AUTOGEN"
...
def otp(request):
    if (request.method == "POST"):
        ...
        url = "http://2factor.in/API/V1/dc4e0f2a-2737-11ee-addf-0200cd936042/SMS/VERIFY/" + request.session['otp_session_data'] + "/" + userotp
        response = requests.request("GET", url)
        data = response.json()
    ...
    if request.method == "POST":
        ...
        if vote.v.has_voted == 'no':
            ...
            if flag==1:
                ...
                url = "http://2factor.in/API/V1/dc4e0f2a-2737-11ee-addf-0200cd936042/SMS/" + vmobno + "/AUTOGEN"
                response = requests.request("GET", url)
                data = response.json()
                request.session['otp_session_data'] = data['Details']
                response_data = {'Message': 'Success'}
